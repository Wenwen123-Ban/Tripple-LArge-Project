...
function renderBookTable(books) { const tbody = document.getElementById('books-tbody'); if(!tbody) return; currentBookRows = books; tbody.innerHTML = books.map((b)=>`<tr><td>${b.book_no}</td><td>${b.title}</td><td>${b.category_name || '—'}</td><td><span class="status-badge status-${(b.computed_status||'available').toLowerCase()}">${b.computed_status || 'Available'}</span>${b.computed_status==='Due'?'<span class="due-tooltip" title="Past due date">⚠</span>':''}</td><td>${b.borrow_count||0}</td><td><button class="btn-row-action" onclick="editBook(${b.id})">Edit</button><button class="btn-row-action" onclick="viewHistory(${b.id})">History</button>${b.computed_status!=='Available'?`<button class="btn-row-action btn-force" onclick="forceReturn(${b.id})">Force Return</button>`:''}<button class="btn-row-action" onclick="notifyBorrower(${b.id})">Notify</button><button class="btn-row-action btn-force" onclick="deleteBook(${b.id})">Delete</button></td></tr>`).join('') || '<tr><td colspan="6">No books found.</td></tr>'; }

async function editBook(bookId){ const current=currentBookRows.find((book)=>book.id===bookId)||{}; const book_no=prompt('Book number:', current.book_no || '')?.trim(); if(!book_no) return; const title=prompt('Book title:', current.title || '')?.trim(); if(!title) return; const category_id=prompt('Category ID (blank for none):', current.category_id || '')?.trim() || null; const status=prompt('Status:', current.computed_status || 'Available')?.trim() || 'Available'; const res=await fetch(`/api/books/${bookId}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({book_no,title,category_id,status})}); const data=await res.json().catch(()=>({})); if(!res.ok){ alert(data.error||'Update failed.'); return; } if(window.showNotification) window.showNotification('Book updated.','success'); await loadBooks(); }
...
async function forceReturn(bookId){ const notes=prompt('Reason for force return (optional):')||''; const res=await fetch('/api/transactions/force-return',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({book_id:bookId,notes})}); const data=await res.json(); window.showNotification?.(data.status==='force_returned'?'Book force-returned successfully.':(data.error||'Force return failed.'), data.status==='force_returned'?'success':'error'); loadBooks(); }
async function notifyBorrower(bookId){ const res=await fetch('/api/transactions/notify-borrower',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({book_id:bookId})}); const data=await res.json(); window.showNotification?.(data.status==='sent'?'Borrower notified by email.':(data.error||'Notification failed.'), data.status==='sent'?'success':'error'); }
