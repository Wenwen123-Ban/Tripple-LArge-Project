...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  document.getElementById('import-commit-btn')?.addEventListener('click', async()=>{const fd=buildImportFormData(); if(!fd) return; const mode=document.getElementById('import-mode').value; if(mode==='dryrun') return; const res=await fetch('/api/books/import/commit',{method:'POST',body:fd}); const data=await res.json(); document.getElementById('import-result').innerHTML=res.ok?`Inserted: ${data.inserted} | Updated: ${data.updated} | Skipped: ${data.skipped}`:(data.error||'Import failed.'); if(res.ok){ bookFilters.status='all'; const sf=document.getElementById('status-filter'); if(sf) sf.value='all'; await loadCategories(); loadBooks(); }});
